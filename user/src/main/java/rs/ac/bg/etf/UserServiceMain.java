...
public class UserServiceMain {
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    try {
      SubmitJobRequest submitRequest =
          SubmitJobRequest.newBuilder()
              .setPaths(
                  JobFilePaths.newBuilder()
      ...
      log.info(
          "SubmitJob: job_id={} status={} message={}",
          submitResponse.getJobId(),
          submitResponse.getStatus(),
          submitResponse.getMessage());

      BagArg bagRequest = BagArg.newBuilder().setNUM(1).build();
      BagRet bagResponse = stub.sendBagTask(bagRequest);
      log.info(
          "SendBagTask: {} result rows, {} task statuses",
          bagResponse.getResultCount(),
          bagResponse.getTaskRetCount());
