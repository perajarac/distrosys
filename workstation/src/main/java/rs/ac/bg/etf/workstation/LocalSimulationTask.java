...
public class LocalSimulationTask implements java.util.concurrent.Callable<SubJobResult> {
    ...
    public SubJobResult call() {
        ...
        SimBufferNetwork network = new SimBufferNetwork(
                request.getJobId(),
        ...
        try {
            Netlist<Object> netlist = NetlistLoader.loadFromLines(
            ...
            rs.ac.bg.etf.sleep.simulation.SimBuffer<Object> queue =
                    (rs.ac.bg.etf.sleep.simulation.SimBuffer<Object>) (rs.ac.bg.etf.sleep.simulation.SimBuffer<?>) network;
            simulator.setQueue(queue);
            simulator.init();
            while (simulator.getlTime() < request.getEndTime()) {
                simulator.execute();
            }
            return SubJobResult.success(WorkstationProtoMapper.toComponentStates(
                    netlist, request.getComponentLinesList()));
        } catch (Exception e) {
            log.error("Simulation failed for job {} sub-job {}", request.getJobId(), request.getSubjobId(), e);
