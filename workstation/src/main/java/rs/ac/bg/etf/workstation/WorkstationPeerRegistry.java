...
public class WorkstationPeerRegistry {
  ...
  private final Map<String, WorkstationServiceGrpc.WorkstationServiceBlockingStub> stubs =
  ...
  public void registerPeers(Map<String, WorkstationEndpoint> endpoints) {
    for (Map.Entry<String, WorkstationEndpoint> entry : endpoints.entrySet()) {
      ...
      channels.computeIfAbsent(
          workstationId,
          id ->
              ManagedChannelBuilder.forAddress(endpoint.getGrpcHost(), endpoint.getGrpcPort())
                  .usePlaintext()
                  .build());
      stubs.computeIfAbsent(
          workstationId, id -> WorkstationServiceGrpc.newBlockingStub(channels.get(id)));
    }
  }
  ...
  public void forwardEvent(String targetWorkstationId, long jobId, Event<Field> event) {
    ...
    ForwardEventRequest request =
        ForwardEventRequest.newBuilder()
            .setJobId(jobId)
