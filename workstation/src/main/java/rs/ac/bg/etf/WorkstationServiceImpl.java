 ...
 *
 * <p>Executes sub-jobs locally via {@link LocalSimulationTask} and accepts forwarded simulation
 * events for distributed partitions.
 */
public class WorkstationServiceImpl extends WorkstationServiceGrpc.WorkstationServiceImplBase {
  ...
  public void executeSubJob(
      SubJobRequest request, StreamObserver<SubJobResponse> responseObserver) {
    ...
    try {
      ...
      if (result.isSuccess()) {
        ...
        builder.setStatus(SubJobStatus.SUB_JOB_FAILED).setErrorMessage(result.getErrorMessage());
      }
      ...
      log.error(
          "Sub-job {} for job {} execution failed", request.getSubjobId(), request.getJobId(), e);
  ...
  public void forwardEvent(ForwardEventRequest request, StreamObserver<EventAck> responseObserver) {
    ...
    EventAck response = EventAck.newBuilder().setAccepted(true).build();
    responseObserver.onNext(response);
    responseObserver.onCompleted();
  }

  private static void respondFailure(
      SubJobRequest request, StreamObserver<SubJobResponse> responseObserver, String message) {
