 ...
 *
 * <p>The netlist maps component IDs to {@link SimComponent} instances and wires source endpoints to
 * destination endpoints. Outgoing events are {@link #transform(List)} into one event per connected
 * destination.
 *
...
public class Netlist<V> {
  ...
  /**
   * Rewrites outgoing events by copying each event to every destination connected to its source
   * endpoint.
   *
  ...
  public List<Event<V>> transform(List<Event<V>> events) {
    ...
    for (Event<V> event : events) {
      List<SimEndpoint> endPoints = getEndpoins(event.srcID, event.srcPort);
   ...
   */
  public void setConnections(HashMap<SimEndpoint, List<SimEndpoint>> connections) {
   ...
   *
   * <p>Expected format: {@code [id, fullyQualifiedClassName, ...stateFields]}. The class must have
   * a no-arg constructor and implement {@link SimComponent}.
   *
  ...
  public void addComponent(String[] data) {
    try {
      ...
      SimComponent<V> component =
          (SimComponent<V>) c.getDeclaredConstructor().newInstance(); // Java 9+
