 ...
 *
 * <p>When an event with logical time earlier than the current {@link #lTime} is dequeued,
 * {@link #restart(long)} is called on every component and processing stops for that step.
 *
...
public class SimulatorOptimistic<T extends Serializable> extends Simulator<T> {
	 ...
	 *
	 * <p>If {@link #lTime} exceeds the event's logical time, all components are restarted
	 * to that time. Otherwise logical time is advanced and valid events are processed.
	 */
	@Override
	public void execute() {
