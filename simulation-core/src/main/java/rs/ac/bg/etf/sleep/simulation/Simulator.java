 ...
 *
 * <p>Maintains a {@link Netlist} of {@link SimComponent} instances, an event {@link SimBuffer}, and
 * timing/audit state. Subclasses implement {@link #execute()} to define how events are dequeued and
 * processed.
 *
...
public abstract class Simulator<V> {
  ...
  public void init() {
    Long[] keys =
        netlist.getComponents().keySet().toArray(new Long[netlist.getComponents().size()]);
    for (Long key : keys) {
   ...
   *
   * <p>Repeatedly calls {@link #execute()} and records wall-clock duration per iteration in {@link
   * #iterationTime}.
   */
