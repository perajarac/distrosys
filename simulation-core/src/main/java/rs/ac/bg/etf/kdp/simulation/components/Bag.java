 ...
 *
 * <p>The Bag (typically component id 1) holds the full body list, creates {@link Field} tasks for
 * {@link #n} workers via {@link #createTasksForWorker()}, and accepts merged results from the
 * {@link Collector}. When all tasks for a step are dispatched, it schedules a self-event to
 * continue the loop.
 */
public class Bag extends G {
   ...
   *
   * <p>External messages update {@link #bodies} and advance {@link #lTime}. The method then emits
   * worker tasks and, when {@link #n} tasks are ready, a self-reschedule event.
   *
   ...
   *
   * <p>Each task covers up to {@link #NUM} bodies and is routed to worker {@code next + 2} on port
   * 0. Worker selection rotates through ports 1..{@link #n}.
   *
