 ...
 *
 * <p>Delegates job lifecycle operations to {@link JobStore}, {@link JobValidator}, {@link
 * JobScheduler}, and {@link WorkstationRegistry}.
 */
public class ServerServiceImpl extends ServerServiceGrpc.ServerServiceImplBase {
  ...
  public void submitJob(
      SubmitJobRequest request, StreamObserver<SubmitJobResponse> responseObserver) {
    ...
    ValidationResult validation =
        jobValidator.validate(paths.getComponentsPath(), paths.getConnectionsPath());
  ...
  public void getJobResult(JobId request, StreamObserver<JobResultResponse> responseObserver) {
    jobStore
        ...
        .ifPresentOrElse(
            job -> {
              JobResultResponse.Builder builder =
                  JobResultResponse.newBuilder().setJobId(job.getId()).setStatus(job.getStatus());
              ...
              try {
                if (job.getStatus() == JobStatus.RUNNING
                  ...
                  jobStore.updateStatus(
                      request.getJobId(), JobStatus.ABORTED, "cancelled by client");
                  ...
                  log.warn(
                      "Cancel rejected for job {} in status {}",
                      request.getJobId(),
                      job.getStatus());
                  responseObserver.onNext(
                      CancelJobResponse.newBuilder()
  ...
  @Override
  public StreamObserver<FileChunk> uploadJobFiles(
      StreamObserver<UploadJobFilesResponse> responseObserver) {
    responseObserver.onError(
        Status.UNIMPLEMENTED.withDescription("not implemented").asRuntimeException());
  ...
  public void registerWorkstation(
    ...
    try {
      ...
      log.info(
          "Registered workstation {} at {}:{} (parallelism={})",
          workstationId,
          request.getGrpcHost(),
          request.getGrpcPort(),
          request.getParallelism());
      RegisterWorkstationResponse response =
          RegisterWorkstationResponse.newBuilder()
